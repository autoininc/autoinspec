...
exports.list = (req, res) => {
    ...
    sql += "SELECT COUNT(a.id) AS cnt" +
        ...
        " WHERE a.category_name LIKE '%" + query_txt +"%'";
...
exports.getlist = (req, res) => {
    ...
    var sql ="SELECT ifnull ( d.id, 0) AS p1_id, d.category_name AS p1_name, ifnull ( c.id, 0) AS p2_id, c.category_name AS p2_name, ifnull ( b.id, 0) AS p3_id, b.category_name AS p3_name, a.category_name AS category_name, a.id as id, a.depth as depth" +
...
exports.addForm = (req, res) => {
    ...
    connection.query('SELECT id, category_name, depth, parent_id, useYN, delYN FROM category WHERE depth = 1 ORDER BY id DESC' , (err,rs) => {   
        if(err) {   
            ...
            model.list = rs;
            res.render('category/add', {model:model});
        }
    });
};
